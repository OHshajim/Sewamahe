...
export const RoomNav = () => {
    ...
    const call = async (isVideo: boolean) => {
        ...
        try {
            ...
            navigate(`/meeting/${res._id}/${type}`, { replace: true });
            await postCall({ roomID: room._id, meetingID: res._id });
        } catch (e) {
    ...
    return (
        <Card className="flex items-center justify-between px-4 py-2 h-14 border-b rounded-none bg-background/70 backdrop-blur-sm">
            {/* Left - Info */}
